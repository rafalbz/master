function piv = distortedpassnew(piv,im1,mask1,im2,mask2,opt)
...
% see SETPIVOPT
  ...
  tic;    
    ...
    for b=1:n_blocks
        ...
        % Define subwindow         
        idx = bsxfun(@plus,(1:W)' - .5 + W/2, piv.x(block)); 
        idy = bsxfun(@plus,(1:H)' - .5 + H/2, piv.y(block));                 
               
        idx = reshape(idx,1,W,block_subwindows);
        idy = reshape(idy,H,1,block_subwindows);
        
        % Extract subwindows
        idx = bsxfun(@plus,(idx-1)*MH,idy);   %??     
        m1 = bsxfun(@times,mask1(idx),window);
        m2 = bsxfun(@times,mask2(idx),window);                              
        f1 = im1(idx);                
        f2 = im2(idx);  
