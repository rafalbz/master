function piv = normalpasspar2(piv,im1,mask1,im2,mask2,opt)
...
% allow indepent masks on ensembles??
  ...
  tic;
    ...
    for b=1:n_blocks
        ...
        if(piv.passes > 1)
          idx2 = bsxfun(@plus, idx2, U(block));
          idy2 = bsxfun(@plus, idy2, V(block));
          idx1 = bsxfun(@minus, idx1, U(block));
          idy1 = bsxfun(@minus, idy1, V(block));                            
        end                                           
                
        % check if indices are out of bound (Need to test this) 
        ido = idx1(1,:) < 1 | idy1(1,:) < 1 | idx1(W1,:) > NN | idy1(H1,:) > MM | ...
              idx2(1,:) < 1 | idy2(1,:) < 1 | idx2(W2,:) > NN | idy2(H2,:) > MM;
