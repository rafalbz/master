function [ncc,mm] = maskednccgpu(f1,m1,f2,m2,~,~,pad)
...
% See Also maskedcc, maskedccj, maskednccj
  ...
  mf = ifft(ifft(bsxfun(@times,M1,F2),m,1),n,2);
  mm = ifft(ifft(bsxfun(@times,M1,M2),m,1),n,2);
  
  num  = ff - bsxfun(@rdivide,fm.*mf,mm);
  den1 = ifft(ifft(bsxfun(@times,F1s,M2),m,1),n,2) - bsxfun(@rdivide,fm.*fm,mm);
  den2 = ifft(ifft(bsxfun(@times,F2s,M1),m,1),n,2) - bsxfun(@rdivide,mf.*mf,mm);
