...
public class DiffMeasure2 
{
    	...
    	if(S==1) {
            for(int i=0; i<idh.length; i++) {
        	    ...
        	    if (alpha==0.0) {
                        ...
                        for(int m=0; m<M; m++) {
                            for(int n=0; n<N; n++) {
				tmp = m1[y0+m][x0+n]*m2[y1+m][x1+n];
                                mm += tmp;	
				ff += tmp*Math.log(Math.abs(f1[y0+m][x0+n] - f2[y1+m][x1+n]));		
                            }
                        } 
                        cc[idh[i]-1][idw[j]-1] = Math.exp(ff/mm); 	
                    } else if(alpha==1.0) {
                        for(int m=0; m<M; m++) {
                            for(int n=0; n<N; n++) {							
                                tmp = m1[y0+m][x0+n]*m2[y1+m][x1+n];
				mm += tmp;
                                ff += tmp*Math.abs(f1[y0+m][x0+n] - f2[y1+m][x1+n]);				
                            }
                        } 
                        cc[idh[i]-1][idw[j]-1] = ff/mm;	
                    } else if(alpha==2.0) {
                        for(int m=0; m<M; m++) {
                            for(int n=0; n<N; n++) {			
                        ...
                        cc[idh[i]-1][idw[j]-1] = Math.sqrt((fm-2*ff+mf)/mm);
                    } else if(alpha==Double.POSITIVE_INFINITY) {
                        for(int m=0; m<M; m++) {
                            for(int n=0; n<N; n++) {
				tmp = m1[y0+m][x0+n]*m2[y1+m][x1+n];
				mm += tmp;
                                tmp *= Math.abs(f1[y0+m][x0+n] - f2[y1+m][x1+n]);
                                ff = (tmp>ff)?tmp:ff; 				                              		                                 
                            }
                        } 
				...
				tmp =  m1[y0+m][x0+n]*m2[y1+m][x1+n];
                                mm += tmp;
                                ff += tmp*mypow(Math.abs(f1[y0+m][x0+n] - f2[y1+m][x1+n]),alpha);				
                            }
                        } 
                        cc[idh[i]-1][idw[j]-1] = mypow(ff/mm,1/alpha);			
                    }
    	    ...
    	    for(int s=0; s<S; s++) {
            	thread[s] = new Thread(new Worker1(s,S));
                thread[s].start();
            }
            for(int s=0; s<S; s++) {	
                try {  
	...
	if(alpha != 0) {
    	...
    	if(S==1) {
            ...
            Thread[] thread = new Thread[S];
    	    for(int s=0; s<S; s++) {
            	thread[s] = new Thread(new Worker2(s,S));
                thread[s].start();
            }
            for(int s=0; s<S; s++) {	
                try {  
    ...
    private class Worker1 implements Runnable 
    {
        ...
        public void run() 
        {
		    ...
		    double mf = 0.0;
				...
				tmp = m1[y0+m][x0+n]*m2[y1+m][x1+n];
                                ff += tmp*Math.abs(f1[y0+m][x0+n] - f2[y1+m][x1+n]);				
                                mf += tmp*Math.abs(f2[y1+m][x1+n]);
                                fm += tmp*Math.abs(f1[y0+m][x0+n]);
                                mmij += tmp;   		
                            }
                        } 
