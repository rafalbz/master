function [pc,mm] = phasecorr3(f1,m1,f2,m2,~,~,~)
% PHASECORR3 computes the phase correlation between
...
%
% [pc,mm] = PHASECORR3(f1,m1,f2,m2,[],[],alpha)
% 
...
% xm = 8;
% idy = (-xm:xm) + 32; 
% [x0,delta,out] = subpixel3x3(pc(idy,idy));
% dx = x0-delta-xm-1
%
% figure;
% imagesc(pc)
...
% See also maskedcc
  ...
  % Calculate Phase correlation
  F1  = fft(fft(f1.*m1,m,1),n,2);
  F2  = fft(fft(f2.*m2,m,1),n,2);   
  ...
  %FF = F1.*F2;     
  %R = FF./abs(FF).^(alpha);  
  num = ifft(ifft(F1.*F2,m,1),n,2);
  den1 = ifft(ifft(abs(F1).^(2).*M2,m,1),n,2);
  den2 = ifft(ifft(M1.*abs(F2).^(2),m,1),n,2);
  
  %pc = ifft(ifft(R,m,1),n,2);         
  pc = num./sqrt(den1.*den2);
  pc = real(pc);
