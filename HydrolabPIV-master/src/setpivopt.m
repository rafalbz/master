...
% some positive value less than one. Some tableau are built in and 
% can be used by setting tableau to 'euler','modeuler','heun',
% 'rk4std' or 'rk43/8'.
% -------------------------------------------------------------------------
%
...
% Use extended subwindows setpivopt('subwindow',[H1 H2],[W1 W2],overlap)?
     ...
     case 'tableau'
      if(ischar(varargin{k+1}))
        switch(lower(varargin{k+1}))
         ...
         otherwise      
          warning('Unknown method "%s"',varargin{k});  
          opt.tableau = [0 0 0 0 0; 1/2 1/2 0 0 0; 1/2 0 1/2 0 0; 1 0 0 1 0; 0 1/6 1/3 1/3 1/6];           
        end
      else          
  ...
  dx = floor(opt.Width*(1-opt.Overlap(1))); %beta
  dy = floor(opt.Height*(1-opt.Overlap(1))); 
  if(dx==0 || dy==0)
