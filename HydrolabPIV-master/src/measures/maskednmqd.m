function [nmqd,mm] = maskednmqd(f1,m1,f2,m2,~,~,pad)
% MASKEDNMQD performes masked normalized minimum quadratic difference on
...
%
% [nmqd,mm] = MASKEDNMQD(f1,m1,f2,m2,[],[],pad)
%
% In additon to the normalized minimum quadratic difference, nmqd, 
% the function also returns the masked fraction, mm. 
% 
% The fifth and sixth argument are for interchangability with maskednmdj
% and are ignored.
% 
% Example
% 
...
% [nmqd,mm] = MASKEDNMQD(A,[],B,[]);
%
% % Find displacement dx
% xm = 8;
% idy = (-xm:xm) + 32; 
% [x0,delta,out] = subpixel3x3(-nmqd(idy,idy));
% dx = x0-delta-xm-1
%
% % plot part of normalized mqd with at least 25% overlap
% figure;
% imagesc(nmqd.*(mm>=.25))
%
% See Also maskedmqd, maskedmdj, maskednmdj
  ...
  % Calculate masked normalized minimum quadratic difference using fft
  ...
  if(pad)
    ...
    r = floor(min(ma,mb)/2);
    s = floor(min(na,nb)/2);
    tmp = circshift(nmqd,m-r,1);
    tmp = circshift(tmp,n-s,2);        
    %mqd = tmp(1:end-1,1:end-1); 
    
    % Resize to match same size as when padded        
    nmqd = zeros(ma+mb-1,na+nb-1,ka,'like',tmp);
    nmqd((1:m-1)+r,(1:n-1)+s,:) = tmp(1:end-1,1:end-1,:);     
  end
