function [md,mm] = maskedmdj(f1,m1,f2,m2,idh,idw,alpha)
% MASKEDMDJ performes masked Minkowski difference on
...
%
% [md,mm] = MASKEDMDJ(f1,m1,f2,m2,idh,idw,alpha)
%
% In additon to the Minkowski-difference, md, the function also returns
% the masked fraction, mm. 
%
% Increase the computational speed by providing the indices idh and idw  
% to partly calculate the Minkowski difference such that only
% md(idh,idw) is calculated. 
...
% xm = 8;
% idy = (-xm:xm) + 32; 
% md = MASKEDMDJ(A,[],B,[],idy,idy,1);
%
% % Find displacement dx
% [x0,delta,out] = subpixel3x3(-md(idy,idy));
% dx = x0-delta-xm-1
%
% % Plot minimum absolute difference
% figure;
% imagesc(md);
%
% See Also maskedmqd, maskednmqd, maskednmdj
...
if(k1==1 && k2==1)
    md = zeros(ma+mb-1,na+nb-1,ka);    
    for k=1:ka    
        md(:,:,k) = dm.maskedmd(f1(:,:,k),m1,f2(:,:,k),m2,idh,idw,alpha);
    end
    mm = dm.getmm();
else
    md = zeros(ma+mb-1,na+nb-1,ka);
    ...
    for k=1:ka    
        md(:,:,k) = dm.maskedmd(f1(:,:,k),m1(:,:,min(k,k1)),f2(:,:,k),m2(:,:,min(k,k2)),idh,idw,alpha);
        mm(:,:,k) = dm.getmm();
    end    
end
