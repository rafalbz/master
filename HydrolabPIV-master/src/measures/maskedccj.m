function [cc,mm] = maskedccj(f1,m1,f2,m2,idh,idw,~)
% MASKEDCCJ performes masked cross-correlation on
...
% to partly calculate the  cross-correlation such that only
% cc(idh,idw) is calculated. 
% The number of computational threads used can be set with DiffMeasure(N).
% 
% The last argument are for interchangability with maskedcc and are ignored.
% 
% Example
% 
...
% imagesc(cc);
%
% See Also maskedncc, maskedcc, maskednccj
...
% Calculate cross-correlation
if(k1==1 && k2==1)
    cc = zeros(ma+mb-1,na+nb-1,ka);    
    for k=1:ka    
        cc(:,:,k) = dm.maskedmd(f1(:,:,k),m1,f2(:,:,k),m2,idh,idw,NaN);
    end
    mm = dm.getmm();
else
    cc = zeros(ma+mb-1,na+nb-1,ka);
    ...
    for k=1:ka    
        cc(:,:,k) = dm.maskedmd(f1(:,:,k),m1(:,:,min(k,k1)),f2(:,:,k),m2(:,:,min(k,k2)),idh,idw,NaN);
        mm(:,:,k) = dm.getmm();
    end    
end
